...
public class LinkedList_RotateClockwiseK {
    ...
    private ListNode rotateClockwiseByK(ListNode node, int k) {
        ...
        k = k % size(node);
        if (k == 0) return node;
        int moveTillDistance = size(node) - k;

        int count = 0;

        ListNode temp = node;

        while (count < moveTillDistance - 1) {
            temp = temp.next;
            count++;
        }
    ...
    public static void main(String[] args) {
        LinkedList_RotateClockwiseK linkedList_rotateClockwise = new LinkedList_RotateClockwiseK();
